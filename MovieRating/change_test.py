...
tests = open('test.txt', 'r').read().split('\n')[1:-1]
new_tests = open('new_tests.txt', 'w')

Genre = movies[-1]
movies = movies[:-1]
new_tests.write(''.join(['Id,Gender,Age,Occupation,Year,', Genre, '\n']))
Genre = Genre.split(',')
for train in tests:
	train = train.split(',')
	Id = train[0]
	...
	new_tests.write(''.join([Id, ',', gender, ',', age, ',', occupation, ',', year, ',', genre_list, '\n']))
